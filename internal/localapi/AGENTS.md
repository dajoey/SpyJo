...
- Read-only `GET /v1/settings` returns the typed catalog through the shared `config.SettingsJSON` projection, with secret values already rendered as state rather than content.
