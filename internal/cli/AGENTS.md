...
- When the election reports the process's own executable was replaced, fail the owner loop loudly (non-zero exit) instead of acquiring and releasing in a retry loop, so the service manager restarts the instance from the new binary. Back repeated non-fatal primary-acquisition failures off exponentially (5s to 60s cap) with at most one log line per deferral so a wedged contender cannot mint a runtime session file every second.
