...
- Render the catalog as JSON through the one shared `SettingsJSON` projection so the loopback settings endpoint and the offline settings-metadata helper cannot drift as catalog fields change.
